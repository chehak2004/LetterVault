...
const ViewLetters = () => {
  ...
  const handleDownload = (fileUrl) => {
    ...
    link.href = fileUrl;
    link.download = fileUrl.split("/").pop(); // optional: auto-detect filename
  ...
  const handleEdit = (letter) => {
    ...
    // You can use a modal or navigate to an edit page here
  };
          ...
          <tbody>
            {filteredLetters.map((letter, index) => (
              <tr key={index} className="bg-white border-t text-center">
                ...
                <td className="px-3 py-2 text-left text-blue-600 space-y-1">
                  <a
                    href={letter.fileUrl}
                    target="_blank"
                    rel="noreferrer"
